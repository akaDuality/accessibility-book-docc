...
Buttons can be not only UIButton elements, but also, for example, cells in lists. If a cell can be tapped, we need to tell that. The easiest way is to set the .button trait on the cell. We'll also discuss lists and cells in detail in the next chapter <doc:13-Cells>.
