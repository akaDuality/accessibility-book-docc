...
После скролла обязательно отправьте оповещение `.pageScrolled` с описанием текущего положения. VoiceOver прочитает этот текст, и пользователь поймёт, куда он проскроллил. Без этого оповещения скролл будет «тихим», и пользователь потеряется в содержимом.
...
> Warning: Это сложная часть для самых опытных, чтобы лучше понять как всё работает изнутри.
